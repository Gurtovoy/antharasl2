            ...
            if (action.equals("create")) {
                try {
                    ...
                    if (target.getPlayer().getLevel() < 10) {
                        activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_DO_NOT_MEET_THE_CRITERIA_IN_ORDER_TO_CREATE_A_CLAN);
                        return false;
                    }
                    String pledgeName = st.nextToken();
                    if (pledgeName.length() > 16) {
                        activeChar.sendPacket((IBroadcastPacket)SystemMsg.CLAN_NAMES_LENGTH_IS_INCORRECT);
                        return false;
                    }
                    if (!Util.isMatchingRegexp(pledgeName, Config.CLAN_NAME_TEMPLATE)) {
                        activeChar.sendPacket((IBroadcastPacket)SystemMsg.CLAN_NAME_IS_INVALID);
                        return false;
                    }
                    ...
                    activeChar.sendPacket((IBroadcastPacket)SystemMsg.THIS_NAME_ALREADY_EXISTS);
                    return false;
                }
                catch (Exception e) {}
                ...
                newLeaderName = st.hasMoreTokens() ? st.nextToken() : target.getName();
