...
import l2s.gameserver.geodata.utils.LinePointIterator;
import l2s.gameserver.geodata.utils.LinePointIterator3D;
...
public class GeodataUtils {
    ...
    public static void debug2DLine(Player player, int x, int y, int tx, int ty, int z) {
        ...
        player.sendPacket((IBroadcastPacket)prim);
    }
        ...
        for (int dx = -geoRadius; dx <= geoRadius; ++dx) {
            for (int dy = -geoRadius; dy <= geoRadius; ++dy) {
                if (iBlock >= 10) {
                    ...
                    if (exsp != null) {
                        ++iPacket;
                        player.sendPacket((IBroadcastPacket)exsp);
                    }
                ...
                Color col = GeodataUtils.getDirectionColor(x, y, z, geoIndex, (byte)8);
                ...
                col = GeodataUtils.getDirectionColor(x, y, z, geoIndex, (byte)1);
                ...
                exsp.addLine("", col, true, x + 4, y - 4, z, x + 4, y + 4, z);
                col = GeodataUtils.getDirectionColor(x, y, z, geoIndex, (byte)4);
                exsp.addLine("", col, true, x - 1, y + 7, z, x + 1, y + 7, z);
                exsp.addLine("S", col, true, x - 2, y + 6, z, x + 2, y + 6, z);
                exsp.addLine("", col, true, x - 3, y + 5, z, x + 3, y + 5, z);
                exsp.addLine("", col, true, x - 4, y + 4, z, x + 4, y + 4, z);
                col = GeodataUtils.getDirectionColor(x, y, z, geoIndex, (byte)2);
                exsp.addLine("", col, true, x - 7, y - 1, z, x - 7, y + 1, z);
