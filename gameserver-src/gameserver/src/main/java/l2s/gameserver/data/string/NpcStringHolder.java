...
public final class NpcStringHolder
extends AbstractHolder {
    ...
    public String getNpcString(Language lang, int npcStringId) {
        TIntObjectMap<String> npcStrings = this._npcStrings.get(lang);
        String name = (String)npcStrings.get(npcStringId);
        if (name == null) {
            Language secondLang = lang;
            while (secondLang != secondLang.getSecondLanguage() && (name = (String)(npcStrings = this._npcStrings.get((secondLang = secondLang.getSecondLanguage()))).get(npcStringId)) == null) {
            }
            if (name == null) {
                for (Language l : Language.VALUES) {
                    npcStrings = this._npcStrings.get(secondLang);
                    name = (String)npcStrings.get(npcStringId);
                    if (name != null) break;
                }
            }
        }
            ...
            try {
                ...
                while (scanner.hasNextLine()) {
                    ...
                    if (m.find()) {
                        ...
                        this._npcStrings.get(lang).put(id, value);
                        continue;
                    }
    ...
    public void log() {
        for (Map.Entry<Language, TIntObjectMap<String>> entry : this._npcStrings.entrySet()) {
            if (!Config.AVAILABLE_LANGUAGES.contains(entry.getKey())) continue;
            this.info("load npc strings: " + entry.getValue().size() + " for lang: " + (Object)(entry.getKey()));
        }
    }

    public int size() {
        return this._npcStrings.size();
    }

    public void clear() {
        this._npcStrings.clear();
    }
}
