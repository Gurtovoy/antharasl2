...
extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (channel != null && channel.getChannelLeader() == player) {
            ...
            player.sendPacket((IBroadcastPacket)SystemMsg.THE_COMMAND_CHANNEL_AFFILIATED_PARTYS_PARTY_MEMBER_CANNOT_USE_THE_MATCHING_SCREEN);
            ...
            if (room != null && room.getType() == MatchingRoom.PARTY_MATCHING) {
                ...
                player.sendPacket((IBroadcastPacket)SystemMsg.THE_LIST_OF_PARTY_ROOMS_CAN_ONLY_BE_VIEWED_BY_A_PERSON_WHO_IS_NOT_PART_OF_A_PARTY);
            }
        } else {
            if (party == null) {
                MatchingRoomManager.getInstance().addToWaitingList(player);
            }
            player.sendPacket((IBroadcastPacket)new ListPartyWaitingPacket(this._region, this._allLevels == 1, this._page, player));
        }
    }
}
