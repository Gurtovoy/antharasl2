...
public class PvPRewardManager {
    ...
    private static boolean basicCheck(Player killed, Player killer) {
        ...
        if (System.currentTimeMillis() / 1000L - killer.getLastAccess() < (long)(Config.PVP_REWARD_MIN_PL_UPTIME_MINUTE * 60000)) {
            if (!no_msg) {
                if (killer.isLangRus()) {
        ...
        if (System.currentTimeMillis() / 1000L - killed.getLastAccess() < (long)(Config.PVP_REWARD_MIN_PL_UPTIME_MINUTE * 60000)) {
            if (!no_msg) {
                if (killer.isLangRus()) {
        ...
        if (killer.getNetConnection() != null && killer.getNetConnection().getHWID() != null && killed.getNetConnection() != null && killed.getNetConnection().getHWID() != null && !Config.PVP_REWARD_SAME_HWID_GIVE && killer.getNetConnection().getHWID().equals(killed.getNetConnection().getHWID())) {
            if (!no_msg) {
                if (killer.isLangRus()) {
        ...
        if (Config.PVP_REWARD_SPECIAL_ANTI_TWINK_TIMER && System.currentTimeMillis() - killed.getCreateTime() < (long)(Config.PVP_REWARD_HR_NEW_CHAR_BEFORE_GET_ITEM * 60000 * 60)) {
            if (!no_msg) {
                if (killer.isLangRus()) {
        ...
        if (Config.PVP_REWARD_CHECK_EQUIP && !PvPRewardManager.checkEquip(killed)) {
            if (!no_msg) {
                if (killer.isLangRus()) {
    ...
    private static boolean checkEquip(Player killed) {
        ...
        return weapon.getGrade().extOrdinal() >= Config.PVP_REWARD_WEAPON_GRADE_TO_CHECK;
    }
    ...
    public static void tryGiveReward(Player victim, Player player) {
        ...
        if (!PvPRewardManager.isNoDelayActive(victim, player)) {
            if (player.isLangRus()) {
        ...
        if (!PvPRewardManager.basicCheck(victim, player)) {
            return;
        }
        victim.setVar("@pvp_manager_" + player.getObjectId(), true, System.currentTimeMillis() + (long)(Config.PVP_REWARD_DELAY_ONE_KILL * 1000));
        PvPRewardManager.giveItem(player);
        if (Config.PVP_REWARD_LOG_KILLS) {
            PvPRewardManager.logCombat(player, victim);
        }
    ...
    private static void giveItem(Player player) {
        ...
        if (Config.PVP_REWARD_RANDOM_ONE) {
            int index = Rnd.get((int)Config.PVP_REWARD_REWARD_IDS.length);
            ...
            for (int i = 0; i < Config.PVP_REWARD_REWARD_IDS.length - 1; ++i) {
                ...
                if (rewardId <= 0 || rewardCount <= 0L) continue;
