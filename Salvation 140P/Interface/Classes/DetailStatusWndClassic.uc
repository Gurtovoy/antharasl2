...
function handleAbilityBtn( bool enable )
{
	local CustomTooltip T;
	local string tmpTooltipString;	
	...
	{
		//버튼 활성
		GetButtonHandle( m_WindowName $ ".abilityIconSlotBlank").HideWindow();
	...
	{	
		...
		GetButtonHandle( m_WindowName $ ".abilityIconSlotBlank").ShowWindow();
		//99레벨 이상, 노블레스일 때..............뭐뭐 하실 수 있습니다.... 라는 메시지 
		tmpTooltipString = GetSystemMessage(4195);
		util.ToopTipInsertText( tmpTooltipString , false, false);
		GetButtonHandle( m_WindowName $ ".abilityIconSlotBlank").SetTooltipCustomType(util.getCustomTooltip());
	}
}*/
...
function InitializeCOD()
{
	...
	//AbilityOpen = GetButtonHandle( "StatusWnd.AbilityOpen" );//ldw

	isDualClass = false;


	txtHeadSTR = GetTextBoxHandle( m_WindowName $ ".txtHeadSTR" );
	txtHeadDEX = GetTextBoxHandle( m_WindowName $ ".txtHeadDEX" );
	txtHeadCON = GetTextBoxHandle( m_WindowName $ ".txtHeadCON" );
	txtHeadINT = GetTextBoxHandle( m_WindowName $ ".txtHeadINT" );
	txtHeadWIT = GetTextBoxHandle( m_WindowName $ ".txtHeadWIT" );
	txtHeadMEN = GetTextBoxHandle( m_WindowName $ ".txtHeadMEN" );
	//txtHeadLUC = GetTextBoxHandle( m_WindowName $ ".txtHeadLUC" );
	//txtHeadCHA = GetTextBoxHandle( m_WindowName $ ".txtHeadCHA" );
...
*/
	...
	// 아레나의 메인 잡 아이콘 변경을 위해 EV_CreatedSubjob 이벤트 전체 주석 해제 - 관련 TT 75064
	else if(Event_ID == EV_CreatedSubjob)
	{	    
		// 메인 잡 아이콘 변경을 위해 아래 함수만 남겨 둠
		updateSubjobInfo(param, Event_ID);
		
		/*
		 * Show 이벤트는 동작 하지 않도록 함.
 ...
 **/
//classic 에서는 메인 job만 있지만, 언제 생길지 모르니 놔둠.//
