...
public class SoulShotItemHandler
extends DefaultItemHandler {
    ...
    public boolean useItem(Playable playable, ItemInstance item, boolean ctrl) {
        ...
        if (player.getActiveWeaponInstance() == null) {
            if (!isAutoSoulShot) {
                player.sendPacket((IBroadcastPacket)SystemMsg.CANNOT_USE_SOULSHOTS);
            }
            return false;
        }
        ...
        if (reducedSoulshot[0] > 0 && Rnd.chance((int)reducedSoulshot[0])) {
        ...
        if (grade != item.getGrade()) {
            ...
            player.sendPacket((IBroadcastPacket)SystemMsg.THE_SOULSHOT_YOU_ARE_ATTEMPTING_TO_USE_DOES_NOT_MATCH_THE_GRADE_OF_YOUR_EQUIPPED_WEAPON);
            return false;
        }
        if (!player.getInventory().destroyItem(item, ssConsumption)) {
            ...
            player.sendPacket((IBroadcastPacket)SystemMsg.YOU_DO_NOT_HAVE_ENOUGH_SOULSHOTS_FOR_THAT);
            return false;
        }
