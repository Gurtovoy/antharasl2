...
extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        switch (this._applyType) {
            case 0: {
                if (!ClanSearchManager.getInstance().removeWaiter(activeChar.getObjectId())) break;
                activeChar.sendPacket((IBroadcastPacket)SystemMsg.ENTRY_APPLICATION_CANCELLED_YOU_MAY_APPLY_TO_A_NEW_CLAN_AFTER_5_MINUTES);
                break;
            }
            case 1: {
                ClanSearchPlayer csPlayer = new ClanSearchPlayer(activeChar.getObjectId(), activeChar.getName(), activeChar.getLevel(), activeChar.getBaseClassId(), this._searchType);
                if (ClanSearchManager.getInstance().addPlayer(csPlayer)) {
                    activeChar.sendPacket((IBroadcastPacket)SystemMsg.ENTERED_INTO_WAITING_LIST_NAME_IS_AUTOMATICALLY_DELETED_AFTER_30_DAYS_IF_DELETE_FROM_WAITING_LIST_IS_USED_YOU_CANNOT_ENTER_NAMES_INTO_THE_WAITING_LIST_FOR_5_MINUTES);
                    break;
                }
