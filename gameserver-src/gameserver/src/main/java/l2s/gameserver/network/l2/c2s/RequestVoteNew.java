...
extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (target.getObjectId() == activeChar.getObjectId()) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_RECOMMEND_YOURSELF);
            return;
        }
        Player targetPlayer = (Player)target;
        if (activeChar.getRecomLeft() <= 0) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_ARE_NOT_AUTHORIZED_TO_MAKE_FURTHER_RECOMMENDATIONS_AT_THIS_TIME);
            return;
        }
        if (targetPlayer.getRecomHave() >= 255) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOUR_SELECTED_TARGET_CAN_NO_LONGER_RECEIVE_A_RECOMMENDATION);
            return;
        }
        ...
        targetPlayer.sendPacket((IBroadcastPacket)sm);
    }
}
