...
extends Skill {
    ...
    public boolean checkCondition(SkillEntry skillEntry, Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first, boolean sendMsg, boolean trigger) {
        ...
        if (this.getNpcId() != 0) {
            ...
            if (player.isInDuel() || player.isSitting() || player.isInCombat() || player.isFishing() || player.isInTrainingCamp() || player.isTransformed() || player.getPet() != null || player.isMounted() || player.isInBoat()) {
                player.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_BOARD_BECAUSE_YOU_DO_NOT_MEET_THE_REQUIREMENTS);
                return false;
            }
    ...
    protected void useSkill(Creature activeChar, Creature target, boolean reflected) {
        ...
        player.setMount(0, this.getNpcId(), player.getLevel(), -1);
    }
}
