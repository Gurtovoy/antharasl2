...
public class MercenaryTicketItemHandler
extends DefaultItemHandler {
    ...
    public void dropItem(Player player, ItemInstance item, long count, Location loc) {
        ...
        if (((SiegeEvent)((Object)castle.getSiegeEvent())).isInProgress()) {
        ...
        for (ItemInstance $item : castle.getSpawnMerchantTickets()) {
            ...
            if ($item.getItemId() != guard.getItemId()) continue;
            ++countOfGuard;
        }
    ...
    public boolean pickupItem(Playable playable, ItemInstance item) {
        ...
        if (!player.hasPrivilege(Privilege.CS_FS_MERCENARIES) || player.getClan().getCastle() == 0) {
            player.sendPacket((IBroadcastPacket)SystemMsg.YOU_DO_NOT_HAVE_THE_AUTHORITY_TO_CANCEL_MERCENARY_POSITIONING);
            return false;
        }
        Castle castle = player.getCastle();
        if (!castle.getSpawnMerchantTickets().contains(item)) {
            player.sendPacket((IBroadcastPacket)SystemMsg.THIS_IS_NOT_A_MERCENARY_OF_A_CASTLE_THAT_YOU_OWN_AND_SO_YOU_CANNOT_CANCEL_ITS_POSITIONING);
            return false;
        }
        if (((SiegeEvent)((Object)castle.getSiegeEvent())).isInProgress()) {
            player.sendPacket(SystemMsg.A_MERCENARY_CAN_BE_ASSIGNED_TO_A_POSITION_FROM_THE_BEGINNING_OF_THE_SEAL_VALIDATION_PERIOD_UNTIL_THE_TIME_WHEN_A_SIEGE_STARTS, ActionFailPacket.STATIC);
