...
extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (activeChar.isInTrade()) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_ARE_ALREADY_TRADING_WITH_SOMEONE);
            return;
        }
        ...
        if (reciever.getBlockList().contains(activeChar)) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_HAVE_BEEN_BLOCKED_FROM_CHATTING_WITH_THAT_CONTACT);
            return;
        }
        if (reciever.getTradeRefusal() || reciever.isBusy()) {
            activeChar.sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.C1_IS_ON_ANOTHER_TASK).addString(reciever.getName()));
            return;
        }
        ...
        new Request(Request.L2RequestType.TRADE_REQUEST, activeChar, reciever).setTimeout(10000L);
        reciever.sendPacket((IBroadcastPacket)new TradeRequestPacket(activeChar.getObjectId()));
        activeChar.sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.YOU_HAVE_REQUESTED_A_TRADE_WITH_C1).addString(reciever.getName()));
    }
}
