...
extends Skill {
    ...
    public Call(StatsSet set) {
        super(set);
        this._force = set.getBool("force_call", false);
    }
    ...
    public boolean checkCondition(SkillEntry skillEntry, Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first, boolean sendMsg, boolean trigger) {
        ...
        if (activeChar.isPlayer()) {
            IBroadcastPacket msg = Call.canSummonHere((Player)activeChar);
            if (msg != null) {
    ...
    protected void useSkill(Creature activeChar, Creature target, boolean reflected) {
        ...
        if (Call.canBeSummoned(target) != null) {
            return;
        }
        Player player = target.getPlayer();
        if (this._force) {
            player.teleToLocation((ILocation)Location.findPointToStay(activeChar, 100, 150), ReflectionManager.MAIN);
        } else {
            player.summonCharacterRequest(activeChar, Location.findAroundPosition(activeChar, 100, 150), this.getId() == 1403 || this.getId() == 1404 ? 1 : 0);
        }
    }
