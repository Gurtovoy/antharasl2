    ...
    public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar) {
        ...
        switch (command) {
            ...
            case admin_vis: {
                ...
                if (activeChar.isGMInvisible()) {
                    ...
                    players = World.getAroundObservers(activeChar);
                    for (Player p : players) {
                        if (!activeChar.isVisible() || activeChar.isInvisible(p)) continue;
                    ...
                    for (Servitor servitor : activeChar.getServitors()) {
                        ...
                        for (Player p : players) {
                            if (!servitor.isVisible() || servitor.isInvisible(p)) continue;
            ...
            case admin_gmspeed: {
                ...
                Creature targetCreature = target != null && target.isCreature() ? (Creature)target : activeChar;
                if (val == 0) {
                    if (targetCreature.isPlayer()) {
                        targetCreature.getPlayer().unsetVar("gm_gmspeed");
                    }
                } else if (targetCreature.isPlayer() && targetCreature.getPlayer().isGM() && Config.SAVE_GM_EFFECTS) {
                    targetCreature.getPlayer().setVar("gm_gmspeed", String.valueOf(val), -1L);
                }
        ...
        switch (command) {
            ...
            case admin_showusm: {
                ...
                activeChar.sendPacket((IBroadcastPacket)new ExShowUsmPacket(usmId));
                break;
            }
            case admin_showchtml: {
                ...
                activeChar.sendPacket((IBroadcastPacket)new TutorialShowHtmlPacket(TutorialShowHtmlPacket.LARGE_WINDOW, "..\\L2text\\" + fileName + ".htm"));
            }
        }
        return true;
    }
