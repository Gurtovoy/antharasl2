...
extends Skill {
    ...
    public Decoy(StatsSet set) {
        ...
        this._numbersOfDecoys = set.getInteger("decoyCount", 1);
    }
    ...
    public void onEndCast(Creature activeChar, Set<Creature> targets) {
        ...
        NpcTemplate DecoyTemplate = NpcHolder.getInstance().getTemplate(this.getNpcId());
        for (int i = 0; i < this._numbersOfDecoys; ++i) {
            DecoyInstance decoy = new DecoyInstance(IdFactory.getInstance().getNextId(), DecoyTemplate, player, this._lifeTime);
