...
extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (!activeChar.isClanLeader()) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.ONLY_THE_CLAN_LEADER_MAY_APPLY_FOR_WITHDRAWAL_FROM_THE_ALLIANCE);
            return;
        }
        if (clan.getAlliance() == null) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_ARE_NOT_CURRENTLY_ALLIED_WITH_ANY_CLANS);
            return;
        }
        if (clan.equals(clan.getAlliance().getLeader())) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.ALLIANCE_LEADERS_CANNOT_WITHDRAW);
            return;
        }
