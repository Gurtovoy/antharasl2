...
public class Config {
    ...
    public static boolean USE_NEW_140_PROTOCOL;
    ...
    public static List<Integer> LIST_OF_SELLABLE_ITEMS;
    public static List<Integer> LIST_OF_TRABLE_ITEMS;
    ...
    // Terrain movement weights for pathfinding
    public static double TERRAIN_LOW_WEIGHT;
    public static double TERRAIN_MEDIUM_WEIGHT;
    public static double TERRAIN_HIGH_WEIGHT;
    public static double DIAGONAL_WEIGHT;
    public static boolean ADVANCED_DIAGONAL_STRATEGY;

    // Spawn height validation
    public static int SPAWN_Z_DELTA_LIMIT;
    public static int SPAWN_HEIGHT_OFFSET;
    ...
    public static boolean WEB_SERVER_ENABLED;
    ...
    public static boolean APPEARANCE_STONE_CHECK_ARMOR_TYPE;
    ...
    public static TIntSet BUFF_STORE_ALLOWED_CLASS_LIST;
    public static TIntSet BUFF_STORE_ALLOWED_SKILL_LIST;
    ...
    public static void loadServerConfig() {
        String[] ignoreAllDropButThis;
        ExProperties serverSettings = Config.load(CONFIGURATION_FILE);
        ...
        double RATE_XP = serverSettings.getProperty("RateXp", 1.0);
        RATE_XP_BY_LVL = new double[127];
        double prevRateXp = RATE_XP;
        for (int i = 1; i < RATE_XP_BY_LVL.length; ++i) {
            double rate;
            Config.RATE_XP_BY_LVL[i] = rate = serverSettings.getProperty("RateXpByLevel" + i, prevRateXp);
            if (rate == prevRateXp) continue;
            prevRateXp = rate;
        }
        double RATE_SP = serverSettings.getProperty("RateSp", 1.0);
        RATE_SP_BY_LVL = new double[127];
        double prevRateSp = RATE_SP;
        for (int i = 1; i < RATE_SP_BY_LVL.length; ++i) {
            double rate;
            Config.RATE_SP_BY_LVL[i] = rate = serverSettings.getProperty("RateSpByLevel" + i, prevRateSp);
            if (rate == prevRateSp) continue;
            prevRateSp = rate;
        }
        MAX_DROP_ITEMS_FROM_ONE_GROUP = serverSettings.getProperty("MAX_DROP_ITEMS_FROM_ONE_GROUP", 1);
        double RATE_DROP_ADENA = serverSettings.getProperty("RateDropAdena", 1.0);
        RATE_DROP_ADENA_BY_LVL = new double[127];
        double prevRateAdena = RATE_DROP_ADENA;
        for (int i = 1; i < RATE_DROP_ADENA_BY_LVL.length; ++i) {
            double rate;
            Config.RATE_DROP_ADENA_BY_LVL[i] = rate = serverSettings.getProperty("RateDropAdenaByLevel" + i, prevRateAdena);
            if (rate == prevRateAdena) continue;
            prevRateAdena = rate;
        }
        double RATE_DROP_ITEMS = serverSettings.getProperty("RateDropItems", 1.0);
        RATE_DROP_ITEMS_BY_LVL = new double[127];
        double prevRateItems = RATE_DROP_ITEMS;
        for (int i = 1; i < RATE_DROP_ITEMS_BY_LVL.length; ++i) {
            double rate;
            Config.RATE_DROP_ITEMS_BY_LVL[i] = rate = serverSettings.getProperty("RateDropItemsByLevel" + i, prevRateItems);
            if (rate == prevRateItems) continue;
            prevRateItems = rate;
        }
        DROP_CHANCE_MODIFIER = serverSettings.getProperty("DROP_CHANCE_MODIFIER", 1.0);
        DROP_COUNT_MODIFIER = serverSettings.getProperty("DROP_COUNT_MODIFIER", 1.0);
        double RATE_DROP_SPOIL = serverSettings.getProperty("RateDropSpoil", 1.0);
        RATE_DROP_SPOIL_BY_LVL = new double[127];
        double prevRateSpoil = RATE_DROP_SPOIL;
        for (int i = 1; i < RATE_DROP_SPOIL_BY_LVL.length; ++i) {
            double rate;
            Config.RATE_DROP_SPOIL_BY_LVL[i] = rate = serverSettings.getProperty("RateDropSpoilByLevel" + i, prevRateSpoil);
            if (rate == prevRateSpoil) continue;
            prevRateSpoil = rate;
        }
        SPOIL_CHANCE_MODIFIER = serverSettings.getProperty("SPOIL_CHANCE_MODIFIER", 1.0);
        SPOIL_COUNT_MODIFIER = serverSettings.getProperty("SPOIL_COUNT_MODIFIER", 1.0);
        ...
        RATE_SP_RAIDBOSS_MODIFIER = serverSettings.getProperty("RATE_SP_RAIDBOSS_MODIFIER", 1.0);
        RATE_DROP_ITEMS_RAIDBOSS = serverSettings.getProperty("RATE_DROP_ITEMS_RAIDBOSS", 1.0);
        DROP_CHANCE_MODIFIER_RAIDBOSS = serverSettings.getProperty("DROP_CHANCE_MODIFIER_RAIDBOSS", 1.0);
        DROP_COUNT_MODIFIER_RAIDBOSS = serverSettings.getProperty("DROP_COUNT_MODIFIER_RAIDBOSS", 1.0);
        RATE_DROP_ITEMS_BOSS = serverSettings.getProperty("RATE_DROP_ITEMS_BOSS", 1.0);
        DROP_CHANCE_MODIFIER_BOSS = serverSettings.getProperty("DROP_CHANCE_MODIFIER_BOSS", 1.0);
        DROP_COUNT_MODIFIER_BOSS = serverSettings.getProperty("DROP_COUNT_MODIFIER_BOSS", 1.0);
        ...
        for (String dropId : ignoreAllDropButThis = serverSettings.getProperty("IgnoreAllDropButThis", "-1").split(";")) {
        ...
        int counter = 0;
        int[] e = serverSettings.getProperty("ABUSEWORD_BAN_CHANNEL", new int[]{0});
        int n = e.length;
        for (int i = 0; i < n; ++i) {
            int id;
            Config.BAN_CHANNEL_LIST[counter] = id = e[i];
            ++counter;
        }
        ...
        if (USE_CLIENT_LANG || CAN_SELECT_LANGUAGE) {
            ...
            for (String availableLanguage : availableLanguages = serverSettings.getProperty("AVAILABLE_LANGUAGES", new String[0], ";")) {
                Language lang = Language.valueOf(availableLanguage.toUpperCase());
                if (lang.isCustom() && !CAN_SELECT_LANGUAGE) continue;
        ...
        WEB_SERVER_ENABLED = serverSettings.getProperty("WebServerEnabled", false);
    ...
    public static void loadTelnetConfig() {
        ExProperties telnetSettings = Config.load(TELNET_CONFIGURATION_FILE);
    ...
    public static void loadResidenceConfig() {
        ExProperties residenceSettings = Config.load(RESIDENCE_CONFIG_FILE);
    ...
    public static void loadChatAntiFloodConfig() {
        ExProperties properties = Config.load(CHAT_ANTIFLOOD_CONFIG_FILE);
    ...
    public static void loadCustomConfig() {
        ExProperties customSettings = Config.load(CUSTOM_CONFIG_FILE);
    ...
    public static void loadOtherConfig() {
        ExProperties otherSettings = Config.load(OTHER_CONFIG_FILE);
    ...
    public static void loadSpoilConfig() {
        ExProperties spoilSettings = Config.load(SPOIL_CONFIG_FILE);
    ...
    public static void loadFormulasConfig() {
        ExProperties formulasSettings = Config.load(FORMULAS_CONFIGURATION_FILE);
        MIN_ABNORMAL_SUCCESS_RATE = formulasSettings.getProperty("MIN_ABNORMAL_SUCCESS_RATE", 10.0);
        MAX_ABNORMAL_SUCCESS_RATE = formulasSettings.getProperty("MAX_ABNORMAL_SUCCESS_RATE", 90.0);
        ...
        ATTACK_TRAIT_POISON_MOD = formulasSettings.getProperty("ATTACK_TRAIT_POISON_MOD", 1.0);
        DEFENCE_TRAIT_POISON_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_POISON_MOD", 1.0);
        ATTACK_TRAIT_HOLD_MOD = formulasSettings.getProperty("ATTACK_TRAIT_HOLD_MOD", 1.0);
        DEFENCE_TRAIT_HOLD_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_HOLD_MOD", 1.0);
        ATTACK_TRAIT_BLEED_MOD = formulasSettings.getProperty("ATTACK_TRAIT_BLEED_MOD", 1.0);
        DEFENCE_TRAIT_BLEED_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_BLEED_MOD", 1.0);
        ATTACK_TRAIT_SLEEP_MOD = formulasSettings.getProperty("ATTACK_TRAIT_SLEEP_MOD", 1.0);
        DEFENCE_TRAIT_SLEEP_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_SLEEP_MOD", 1.0);
        ATTACK_TRAIT_SHOCK_MOD = formulasSettings.getProperty("ATTACK_TRAIT_SHOCK_MOD", 1.0);
        DEFENCE_TRAIT_SHOCK_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_SHOCK_MOD", 1.0);
        ATTACK_TRAIT_DERANGEMENT_MOD = formulasSettings.getProperty("ATTACK_TRAIT_DERANGEMENT_MOD", 1.0);
        DEFENCE_TRAIT_DERANGEMENT_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_DERANGEMENT_MOD", 1.0);
        ATTACK_TRAIT_PARALYZE_MOD = formulasSettings.getProperty("ATTACK_TRAIT_PARALYZE_MOD", 1.0);
        DEFENCE_TRAIT_PARALYZE_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_PARALYZE_MOD", 1.0);
        ATTACK_TRAIT_BOSS_MOD = formulasSettings.getProperty("ATTACK_TRAIT_BOSS_MOD", 1.0);
        DEFENCE_TRAIT_BOSS_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_BOSS_MOD", 1.0);
        ATTACK_TRAIT_DEATH_MOD = formulasSettings.getProperty("ATTACK_TRAIT_DEATH_MOD", 1.0);
        DEFENCE_TRAIT_DEATH_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_DEATH_MOD", 1.0);
        ATTACK_TRAIT_ROOT_PHYSICALLY_MOD = formulasSettings.getProperty("ATTACK_TRAIT_ROOT_PHYSICALLY_MOD", 1.0);
        DEFENCE_TRAIT_ROOT_PHYSICALLY_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_ROOT_PHYSICALLY_MOD", 1.0);
        ATTACK_TRAIT_TURN_STONE_MOD = formulasSettings.getProperty("ATTACK_TRAIT_TURN_STONE_MOD", 1.0);
        DEFENCE_TRAIT_TURN_STONE_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_TURN_STONE_MOD", 1.0);
        ATTACK_TRAIT_GUST_MOD = formulasSettings.getProperty("ATTACK_TRAIT_GUST_MOD", 1.0);
        DEFENCE_TRAIT_GUST_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_GUST_MOD", 1.0);
        ATTACK_TRAIT_PHYSICAL_BLOCKADE_MOD = formulasSettings.getProperty("ATTACK_TRAIT_PHYSICAL_BLOCKADE_MOD", 1.0);
        DEFENCE_TRAIT_PHYSICAL_BLOCKADE_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_PHYSICAL_BLOCKADE_MOD", 1.0);
        ATTACK_TRAIT_TARGET_MOD = formulasSettings.getProperty("ATTACK_TRAIT_TARGET_MOD", 1.0);
        DEFENCE_TRAIT_TARGET_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_TARGET_MOD", 1.0);
        ATTACK_TRAIT_PHYSICAL_WEAKNESS_MOD = formulasSettings.getProperty("ATTACK_TRAIT_PHYSICAL_WEAKNESS_MOD", 1.0);
        DEFENCE_TRAIT_PHYSICAL_WEAKNESS_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_PHYSICAL_WEAKNESS_MOD", 1.0);
        ATTACK_TRAIT_MAGICAL_WEAKNESS_MOD = formulasSettings.getProperty("ATTACK_TRAIT_MAGICAL_WEAKNESS_MOD", 1.0);
        DEFENCE_TRAIT_MAGICAL_WEAKNESS_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_MAGICAL_WEAKNESS_MOD", 1.0);
        ATTACK_TRAIT_KNOCKBACK_MOD = formulasSettings.getProperty("ATTACK_TRAIT_KNOCKBACK_MOD", 1.0);
        DEFENCE_TRAIT_KNOCKBACK_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_KNOCKBACK_MOD", 1.0);
        ATTACK_TRAIT_KNOCKDOWN_MOD = formulasSettings.getProperty("ATTACK_TRAIT_KNOCKDOWN_MOD", 1.0);
        DEFENCE_TRAIT_KNOCKDOWN_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_KNOCKDOWN_MOD", 1.0);
        ATTACK_TRAIT_PULL_MOD = formulasSettings.getProperty("ATTACK_TRAIT_PULL_MOD", 1.0);
        DEFENCE_TRAIT_PULL_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_PULL_MOD", 1.0);
        ATTACK_TRAIT_HATE_MOD = formulasSettings.getProperty("ATTACK_TRAIT_HATE_MOD", 1.0);
        DEFENCE_TRAIT_HATE_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_HATE_MOD", 1.0);
        ATTACK_TRAIT_AGGRESSION_MOD = formulasSettings.getProperty("ATTACK_TRAIT_AGGRESSION_MOD", 1.0);
        DEFENCE_TRAIT_AGGRESSION_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_AGGRESSION_MOD", 1.0);
        ATTACK_TRAIT_AIRBIND_MOD = formulasSettings.getProperty("ATTACK_TRAIT_AIRBIND_MOD", 1.0);
        DEFENCE_TRAIT_AIRBIND_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_AIRBIND_MOD", 1.0);
        ATTACK_TRAIT_DISARM_MOD = formulasSettings.getProperty("ATTACK_TRAIT_DISARM_MOD", 1.0);
        DEFENCE_TRAIT_DISARM_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_DISARM_MOD", 1.0);
        ATTACK_TRAIT_DEPORT_MOD = formulasSettings.getProperty("ATTACK_TRAIT_DEPORT_MOD", 1.0);
        DEFENCE_TRAIT_DEPORT_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_DEPORT_MOD", 1.0);
        ATTACK_TRAIT_CHANGEBODY_MOD = formulasSettings.getProperty("ATTACK_TRAIT_CHANGEBODY_MOD", 1.0);
        DEFENCE_TRAIT_CHANGEBODY_MOD = formulasSettings.getProperty("DEFENCE_TRAIT_CHANGEBODY_MOD", 1.0);
    }

    public static void loadDevelopSettings() {
        ExProperties properties = Config.load(DEVELOP_FILE);
        USE_NEW_140_PROTOCOL = properties.getProperty("USE_NEW_140_PROTOCOL", true);
    }

    public static void loadExtSettings() {
        ExProperties properties = Config.load(EXT_FILE);
    ...
    public static void loadBBSSettings() {
        ExProperties properties = Config.load(BBS_FILE);
    ...
    public static void loadAltSettings() {
        int i;
        String[] stringArray;
        String[] autoLootItemIdList;
        ExProperties altSettings = Config.load(ALT_SETTINGS_FILE);
        ...
        for (String item : autoLootItemIdList = altSettings.getProperty("AutoLootItemIdList", "-1").split(";")) {
        ...
        for (String info : stringArray = altSettings.getProperty("RaidGlobalDrop", new String[0], ";")) {
        ...
        for (i = 1; i < CLAN_LVL_UP_SP_COST.length; ++i) {
            Config.CLAN_LVL_UP_SP_COST[i] = altSettings.getProperty("CLAN_LVL_UP_SP_COST_" + i, 0);
        }
        CLAN_LVL_UP_RP_COST = new int[CLAN_MAX_LEVEL + 1];
        for (i = 1; i < CLAN_LVL_UP_RP_COST.length; ++i) {
            Config.CLAN_LVL_UP_RP_COST[i] = altSettings.getProperty("CLAN_LVL_UP_RP_COST_" + i, 0);
        }
        CLAN_LVL_UP_MIN_MEMBERS = new int[CLAN_MAX_LEVEL + 1];
        for (i = 1; i < CLAN_LVL_UP_MIN_MEMBERS.length; ++i) {
            Config.CLAN_LVL_UP_MIN_MEMBERS[i] = altSettings.getProperty("CLAN_LVL_UP_MIN_MEMBERS_" + i, 1);
        }
        CLAN_LVL_UP_ITEMS_REQUIRED = new long[CLAN_MAX_LEVEL + 1][][][];
        for (i = 1; i < CLAN_LVL_UP_ITEMS_REQUIRED.length; ++i) {
            String[] itemsByLvlVariations = altSettings.getProperty("CLAN_LVL_UP_ITEMS_REQUIRED_" + i, "0-0").split("\\|");
            Config.CLAN_LVL_UP_ITEMS_REQUIRED[i] = new long[itemsByLvlVariations.length][][];
            for (int j = 0; j < itemsByLvlVariations.length; ++j) {
                Config.CLAN_LVL_UP_ITEMS_REQUIRED[i][j] = StringArrayUtils.stringToLong2X((String)itemsByLvlVariations[j], (String)";", (String)"-");
            }
        }
        CLAN_LVL_UP_NEED_CASTLE = new boolean[CLAN_MAX_LEVEL + 1];
        for (i = 1; i < CLAN_LVL_UP_NEED_CASTLE.length; ++i) {
            Config.CLAN_LVL_UP_NEED_CASTLE[i] = altSettings.getProperty("CLAN_LVL_UP_NEED_CASTLE_" + i, false);
        }
        ...
        for (int i2 = 1; i2 < PERCENT_LOST_ON_DEATH.length; ++i2) {
            double percent;
            Config.PERCENT_LOST_ON_DEATH[i2] = percent = altSettings.getProperty("PERCENT_LOST_ON_DEATH_LVL_" + i2, prevPercentLost);
            if (percent == prevPercentLost) continue;
        ...
        ALLOWED_TRADE_ZONES = StringUtils.isEmpty((CharSequence)allowedTradeZones) ? new String[0] : allowedTradeZones.split(";");
    ...
    public static void loadServicesSettings() {
        ExProperties servicesSettings = Config.load(SERVICES_FILE);
        ...
        if (!allowClassMasters.equalsIgnoreCase("false")) {
            String[] allowClassLvls;
            for (String allowClassLvl : allowClassLvls = allowClassMasters.split(";")) {
                String[] allosClassLvlInfo = allowClassLvl.split(",");
                int classLvl = Integer.parseInt(allosClassLvlInfo[0]);
                if (ALLOW_CLASS_MASTERS_LIST.containsKey(classLvl)) continue;
    ...
    public static void loadPvPSettings() {
        ExProperties pvpSettings = Config.load(PVP_CONFIG_FILE);
        ...
        NORMAL_DROPCHANCE_BASE = pvpSettings.getProperty("ChanceOfNormalDropBase", 30.0);
    ...
    public static void loadAISettings() {
        ExProperties aiSettings = Config.load(AI_CONFIG_FILE);
    ...
    public static void loadGeodataSettings() {
        ExProperties geodataSettings = Config.load(GEODATA_CONFIG_FILE);
        ...
        TERRAIN_LOW_WEIGHT = geodataSettings.getProperty("TerrainLowWeight", 0.5);
        TERRAIN_MEDIUM_WEIGHT = geodataSettings.getProperty("TerrainMediumWeight", 2.0);
        TERRAIN_HIGH_WEIGHT = geodataSettings.getProperty("TerrainHighWeight", 3.0);
        DIAGONAL_WEIGHT = geodataSettings.getProperty("DiagonalWeight", 1.414);
        ADVANCED_DIAGONAL_STRATEGY = geodataSettings.getProperty("AdvancedDiagonalStrategy", true);
        SPAWN_Z_DELTA_LIMIT = geodataSettings.getProperty("SpawnZDeltaLimit", 100);
        SPAWN_HEIGHT_OFFSET = geodataSettings.getProperty("SpawnHeightOffset", 20);
    }

    public static void pvpManagerSettings() {
        ExProperties pvp_manager = Config.load(PVP_MANAGER_FILE);
    ...
    public static void loadOlympiadSettings() {
        ExProperties olympSettings = Config.load(OLYMPIAD);
    ...
    public static void load() {
        ...
        Config.loadChatAntiFloodConfig();
        ...
        Config.loadDevelopSettings();
        Config.loadExtSettings();
        Config.loadBBSSettings();
        Config.loadSchemeBuffer();
        Config.loadBuffStoreConfig();
        Config.loadTrainingCampConfig();
        Config.abuseLoad();
        Config.loadGMAccess();
        Config.pvpManagerSettings();
        Config.loadAntiBotSettings();
    }
    ...
    public static void abuseLoad() {
        ...
        try {
            ...
            lnr = new LineNumberReader(new InputStreamReader((InputStream)new FileInputStream(ANUSEWORDS_CONFIG_FILE), "UTF-8"));
            int count = 0;
    ...
    public static void loadAntiBotSettings() {
        ExProperties botSettings = Config.load(BOT_FILE);
    ...
    public static void loadSchemeBuffer() {
        ExProperties npcbuffer = Config.load(SCHEME_BUFFER_FILE);
    ...
    public static void loadBuffStoreConfig() {
        ExProperties buffStoreConfig = Config.load(BUFF_STORE_CONFIG_FILE);
        ...
        BUFF_STORE_ALLOWED_CLASS_LIST.addAll(StringArrayUtils.stringToIntArray((String)buffStoreConfig.getProperty("BuffStoreAllowedClassList", ""), (String)","));
        BUFF_STORE_ALLOWED_SKILL_LIST.addAll(StringArrayUtils.stringToIntArray((String)buffStoreConfig.getProperty("BUFF_STORE_ALLOWED_SKILL_LIST", ""), (String)","));
    ...
    public static void loadTrainingCampConfig() {
        ExProperties properties = Config.load(TRAINING_CAMP_CONFIG_FILE);
    ...
    public static void loadGMAccess() {
        ...
        for (File f : dir.listFiles()) {
            if (f.isDirectory() || !f.getName().endsWith(".xml")) continue;
            Config.loadGMAccess(f);
        }
    }
            ...
            for (Node z = doc.getFirstChild(); z != null; z = z.getNextSibling()) {
                for (Node n = z.getFirstChild(); n != null; n = n.getNextSibling()) {
                    ...
                    for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling()) {
                        ...
                        if (!fld.getType().getName().equalsIgnoreCase("int")) continue;
    ...
    public static String getField(String fieldName) {
        Field field = FieldUtils.getField(Config.class, (String)fieldName);
        if (field == null) {
            return null;
        }
        try {
        ...
        block9: {
            ...
            try {
                if (field.getType() == Boolean.TYPE) {
                    field.setBoolean(null, BooleanUtils.toBoolean((String)value));
                    break block9;
                }
                if (field.getType() == Integer.TYPE) {
                    field.setInt(null, NumberUtils.toInt((String)value));
                    break block9;
                }
                if (field.getType() == Long.TYPE) {
                    field.setLong(null, NumberUtils.toLong((String)value));
                    break block9;
                }
                if (field.getType() == Double.TYPE) {
                    field.setDouble(null, NumberUtils.toDouble((String)value));
                    break block9;
                }
    ...
    static {
        ...
        KARMA_LIST_NONDROPPABLE_ITEMS = new ArrayList<Integer>();
        RAID_GLOBAL_DROP = new ArrayList<RaidGlobalDrop>();
        LIST_OF_SELLABLE_ITEMS = new ArrayList<Integer>();
        LIST_OF_TRABLE_ITEMS = new ArrayList<Integer>();
