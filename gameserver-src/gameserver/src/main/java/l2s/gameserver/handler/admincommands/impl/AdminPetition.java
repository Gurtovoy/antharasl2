...
implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar) {
        ...
        int petitionId = NumberUtils.toInt((String)(wordList.length > 1 ? wordList[1] : "-1"), (int)-1);
        ...
        switch (command) {
            ...
            case admin_accept_petition: {
                ...
                if (PetitionManager.getInstance().isPlayerInConsultation(activeChar)) {
                    activeChar.sendPacket((IBroadcastPacket)new SystemMessage(390));
                    return true;
                }
                if (PetitionManager.getInstance().isPetitionInProcess(petitionId)) {
                    activeChar.sendPacket((IBroadcastPacket)new SystemMessage(407));
                    return true;
                }
                if (PetitionManager.getInstance().acceptPetition(activeChar, petitionId)) break;
                activeChar.sendPacket((IBroadcastPacket)new SystemMessage(388));
                break;
            }
            case admin_reject_petition: {
                ...
                if (!PetitionManager.getInstance().rejectPetition(activeChar, petitionId)) {
                    activeChar.sendPacket((IBroadcastPacket)new SystemMessage(393));
                }
                PetitionManager.getInstance().sendPendingPetitionList(activeChar);
                break;
            }
