...
function cancelBtnClick()
{
	...
	switch(currentWindowName)
	{
		case "CharacterPasswordWnd_New"    :  Me.HideWindow();// targetWindowShow("CharacterPasswordWnd_Normal");											  
											  // Me.HideWindow();
											  LobbyMenuWnd(GetScript("LobbyMenuWnd")).isStarting = false;
											  break;
		...
		case "CharacterPasswordWnd_Normal" :  // targetWindowShow("CharacterPasswordWnd_Normal");						  
											  ExecuteEvent( EV_LobbyCharacterReceivingFinished );
											  LobbyMenuWnd(GetScript("LobbyMenuWnd")).isStarting = false;
											  Me.HideWindow();
											  break;
	}
...
function OnEvent( int Event_ID, string param )
{
	...
	switch( Event_ID )
	{
		...
		case EV_SecondaryAuthVerify     : randomWindow();
										  Me.ShowWindow();
										  Me.SetFocus();
		...
		case EV_SecondaryAuthBlocked    : ParseInt(param, "BlockTime", blockTime );
										  ...
										  DialogShow(DialogModalType_Modal, DialogType_OK, MakeFullSystemMsg(GetSystemMessage(3376), getTimeStringBySec(blockTime, true, true)), string(self));
										  break;
		...
		case EV_SecondaryAuthSuccess    : Me.HideWindow();
										  ...
										  if (result == 0 || result == 2)
										  {
											  ...
											  DialogSetReservedInt2(0);
											  DialogSetID( DIALOG_SAVE_SUCCESS_PASSWORD );
											  DialogShow(DialogModalType_Modal, DialogType_OK, GetSystemMessage(3322), string(self));
										  }
										  break;
		...
		case EV_SecondaryAuthVerifyFail : ParseInt(param, "Result", result );
										  ...
										  DialogSetReservedInt2(count);
										  DialogSetID( DIALOG_WRONG_PASSWORD );										  
										  DialogShow(DialogModalType_Modal, DialogType_OK, MakeFullSystemMsg( GetSystemMessage(3315), string(count)), string(self));
										  break;
		...
		//									 DialogSetReservedInt(result);
		//								     DialogSetID( DIALOG_WRONG_PASSWORD );
		//									 DialogShow(DialogModalType_Modal, DialogType_OK, GetSystemMessage(3416), string(self));
		//								  }
		//								  break;
			
		case EV_SecondaryAuthFailEtc    : ParseInt(param, "ErrorCode", errorCode );
										  DialogSetID( DIALOG_ETCERROR_PASSWORD );
										  DialogShow(DialogModalType_Modal, DialogType_OK, MakeFullSystemMsg( GetSystemMessage(3391), string(errorCode)), string(self));
										  break;
	}
...
function HandleDialogOK()
{
	...
	if (DialogIsMine())
	{
		...
		switch( DialogGetID() )
		{
			...
			case DIALOG_WRONG_PASSWORD        : if (DialogGetReservedInt2() >= 5) Me.hideWindow(); 
	...
	switch(currentWindowName)
	{
		...
		case "CharacterPasswordWnd_Edit"   : if (getPasswordString(InputEdit2) == getPasswordString(InputEdit3))
											 ...
											 else
											 {
												 ...
												 DialogShow(DialogModalType_Modalless, DialogType_OK, GetSystemMessage( 3318 ), string(self) );
