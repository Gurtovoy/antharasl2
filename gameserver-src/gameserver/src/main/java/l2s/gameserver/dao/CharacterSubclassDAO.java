...
public class CharacterSubclassDAO {
    ...
    public boolean insert(int objId, int classId, long exp, long sp, double curHp, double curMp, double curCp, double maxHp, double maxMp, double maxCp, int level, boolean active, SubClassType type) {
        ...
        try {
            ...
            boolean bl;
            try {
                _log.error("CharacterSubclassDAO:insert(player)", (Throwable)e);
                bl = false;
            }
    ...
    public List<SubClass> restore(Player player) {
        ...
        try {
            ...
            try {
                _log.error("CharacterSubclassDAO:restore(player)", (Throwable)e);
            }
            catch (Throwable throwable) {
    ...
    public boolean store(Player player) {
        ...
        try {
            ...
            boolean bl;
            try {
                _log.error("CharacterSubclassDAO:store(player)", (Throwable)e);
                bl = false;
            }
            catch (Throwable throwable) {
                DbUtils.closeQuietly((Connection)con, statement);
