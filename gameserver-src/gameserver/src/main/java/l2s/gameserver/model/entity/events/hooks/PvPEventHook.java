...
extends ListenerHook {
    ...
    public void onPlayerDie(Player player, Creature killer) {
        ...
        PvPEventPlayerObject member = arena.getParticipant(player);
        if (member == null) {
            return;
        }
        member.addCountDie();
        ...
        rewards.stream().filter(reward -> Rnd.chance((double)reward.getChance())).forEach(reward -> ItemFunctions.addItem(killer.getPlayer(), reward.getItemId(), Rnd.get((long)reward.getMinCount(), (long)reward.getMaxCount()) * (long)(killer.getPlayer().hasPremiumAccount() ? event.getModRewardForPremium() : 1)));
    }
