...
implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar) {
        ...
        if (fullString.startsWith("admin_find_mammon")) {
            ...
            this.findAdminNPCs(activeChar, this.npcIds, teleportIndex, -1);
            ...
            activeChar.sendPacket((IBroadcastPacket)new SystemMessage(Integer.parseInt(fullString.substring(10).trim())));
        }
        return true;
    }
    ...
    public void findAdminNPCs(Player activeChar, List<Integer> npcIdList, int teleportIndex, int makeVisible) {
        ...
        for (NpcInstance npcInst : GameObjectsStorage.getNpcs()) {
            ...
            if (!npcIdList.contains(npcId)) continue;
            if (makeVisible == 1) {
                npcInst.spawnMe();
            } else if (makeVisible == 0) {
                npcInst.decayMe();
            }
            if (!npcInst.isVisible()) continue;
            ++index;
            if (teleportIndex > -1) {
