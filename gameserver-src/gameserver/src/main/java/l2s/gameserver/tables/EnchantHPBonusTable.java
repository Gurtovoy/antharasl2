...
public class EnchantHPBonusTable {
            ...
            for (Node n = doc.getFirstChild(); n != null; n = n.getNextSibling()) {
                ...
                for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling()) {
                    ...
                    if ("options".equalsIgnoreCase(d.getNodeName())) {
                        ...
                        this._onepieceFactor = Integer.parseInt(att.getNodeValue());
                        continue;
                    }
                    if (!"enchant_bonus".equalsIgnoreCase(d.getNodeName())) continue;
                    att = attrs.getNamedItem("grade");
                    if (att == null) {
                        _log.info("EnchantHPBonusTable: Missing grade, skipping");
                        continue;
                    }
                    ItemGrade grade = ItemGrade.valueOf(att.getNodeValue());
                    ...
                    this._armorHPBonus.put(grade.ordinal(), bonus);
                }
            }
            _log.info("EnchantHPBonusTable: Loaded bonuses for " + this._armorHPBonus.size() + " grades.");
        }
        catch (Exception e) {
    ...
    public final int getHPBonus(Player player, ItemInstance item) {
        ...
        Integer[] values = (Integer[])this._armorHPBonus.get(item.getTemplate().getGrade().ordinal());
        if (values == null || values.length == 0) {
            return 0;
        }
        int bonus = values[Math.min(item.getFixedEnchantLevel(player), values.length) - 1];
        if (item.getTemplate().getBodyPart() == 32768L) {
            bonus = (int)((double)(bonus * this._onepieceFactor) / 100.0);
        }
        return bonus;
    }
}
