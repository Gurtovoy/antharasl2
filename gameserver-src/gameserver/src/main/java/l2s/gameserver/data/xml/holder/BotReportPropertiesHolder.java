package l2s.gameserver.data.xml.holder;

import gnu.trove.map.TIntObjectMap;
import gnu.trove.map.hash.TIntObjectHashMap;
import java.util.Collection;
import l2s.commons.data.xml.AbstractHolder;
import l2s.gameserver.templates.BotPunishment;

public final class BotReportPropertiesHolder
extends AbstractHolder {
    private static final BotReportPropertiesHolder _instance = new BotReportPropertiesHolder();
    private final TIntObjectMap<BotPunishment> _punishments = new TIntObjectHashMap();

    public static BotReportPropertiesHolder getInstance() {
        return _instance;
    }

    public void addBotPunishment(BotPunishment punishment) {
        this._punishments.put(punishment.getNeedReportPoints(), punishment);
    }

    public BotPunishment getBotPunishment(int needReportPoints) {
        return (BotPunishment)this._punishments.get(needReportPoints);
    }

    public Collection<BotPunishment> getBotPunishments() {
        return this._punishments.valueCollection();
    }

    public int size() {
        return this._punishments.size();
    }

    public void clear() {
        this._punishments.clear();
    }
}

