...
public class DDSConverter {
    public static ByteBuffer convertToDxt1NoTransparency(BufferedImage bufferedimage) {
        ...
        for (int l = 0; l < k; ++l) {
            for (int i1 = 0; i1 < j; ++i1) {
                ...
                Color[] acolor = DDSConverter.getColors888(ai);
                for (int j1 = 0; j1 < ai.length; ++j1) {
                    ai[j1] = DDSConverter.getPixel565(acolor[j1]);
                    acolor[j1] = DDSConverter.getColor565(ai[j1]);
                }
                int[] ai1 = DDSConverter.determineExtremeColors(acolor);
                ...
                long l1 = DDSConverter.computeBitMask(acolor, ai1);
    ...
    private static int[] determineExtremeColors(Color[] acolor) {
        ...
        for (int j = 0; j < acolor.length - 1; ++j) {
            for (int k = j + 1; k < acolor.length; ++k) {
                int l = DDSConverter.distance(acolor[j], acolor[k]);
                if (l <= i) continue;
                i = l;
                ai[0] = j;
                ai[1] = k;
    ...
    private static long computeBitMask(Color[] acolor, int[] ai) {
        Color[] acolor1 = new Color[]{null, null, new Color(), new Color()};
        ...
        for (int i = 0; i < acolor.length; ++i) {
            ...
            for (int i1 = 0; i1 < acolor1.length; ++i1) {
                int j1 = DDSConverter.distance(acolor[i], acolor1[i1]);
                if (j1 >= j) continue;
                j = j1;
                k = i1;
            }
