...
object AutobotsManager {
    ...
    private lateinit var onUpdateKlock: Klock
    ...
    fun start(){
        ...
        onUpdateKlock = Klock.start("onUpdate", 1000, AutobotData.settings.iterationDelay, CoScopes.onUpdateScope, action = {
    ...
    fun onServerShutdown(){
        onUpdateKlock.cancel()
        AutobotScheduler.shutdown()
        AutobotsDao.updateAllBotOnlineStatus(false)
        CoScopes.shutdown()
    }
    ...
    internal suspend fun spawnAutobot(autobot: Autobot){
        ...
        try {
            ...
            activeBots.remove(autobot.name)
            println("Failed to spawn autobot ${autobot.name}: $e")
            throw e
        }
    }
