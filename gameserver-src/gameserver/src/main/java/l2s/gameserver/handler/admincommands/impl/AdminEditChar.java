...
implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar) {
        ...
        if (fullString.equals("admin_current_player")) {
            AdminEditChar.showCharacterList(activeChar, null);
            ...
            try {
                ...
                AdminEditChar.showCharacterList(activeChar, target);
            }
            catch (StringIndexOutOfBoundsException e) {}
            ...
            if (fullString.equals("admin_edit_character")) {
                ...
                try {
                    ...
                    boolean addExp = true;
                    boolean addSp = false;
                    for (String value : vals = val.split(" ")) {
                        if (Strings.isDigit(value)) {
                            if (addExp) {
                                exp = NumberUtils.toLong((String)value, (long)0L);
                                addExp = false;
                                continue;
                            }
                            if (!addSp && addExp) continue;
                            sp = NumberUtils.toInt((String)value, (int)0);
                            addSp = false;
                            continue;
                        }
                        ...
                        if (!value.equalsIgnoreCase("sp")) continue;
                        addExp = false;
                        addSp = true;
                    }
                ...
                try {
                    ...
                    this.setTargetFame(activeChar, fame);
                }
                catch (StringIndexOutOfBoundsException e) {
                ...
                target.sendPacket((IBroadcastPacket)new ExPCCafePointInfoPacket(target, count, 1, 2, 12));
    ...
    private void listCharacters(Player activeChar, int page) {
        ArrayList<Player> players = new ArrayList<Player>(GameObjectsStorage.getPlayers(true, true));
    ...
    public static void showCharacterList(Player activeChar, Player player) {
        ...
        replyMSG.append("<tr><td width=100>Direction:</td><td>" + (Object)((Object)PositionUtils.getDirectionTo(player, activeChar)) + "</td></tr>");
        replyMSG.append("<tr><td width=100>Intention:</td><td>" + (Object)((Object)player.getAI().getIntention()) + "</td></tr>");
        replyMSG.append("</table><br>");
    ...
    private void adminModifyCharacter(Player activeChar, String modifications) {
        GameObject target = activeChar.getTarget();
        if (target == null || !target.isPlayer()) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.SELECT_TARGET);
            return;
        }
        Player player = (Player)target;
    ...
    private void editCharacter(Player activeChar) {
        ...
        replyMSG.append("</table>");
        replyMSG.append("<table width=270><tr><td>Class<?> Template Id: " + (Object)((Object)player.getClassId()) + "/" + player.getClassId().getId() + "</td></tr></table><br>");
        replyMSG.append("<table width=270>");
    ...
    private void findCharacter(Player activeChar, String CharacterToFind) {
        ...
        for (Player element : GameObjectsStorage.getPlayers(true, true)) {
            if (!element.getName().startsWith(CharacterToFind)) continue;
            ++CharactersFound;
            replyMSG.append("<table width=270>");
            replyMSG.append("<tr><td width=80>Name</td><td width=110>Class</td><td width=40>Level</td></tr>");
