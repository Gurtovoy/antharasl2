...
implements DataSource {
    ...
    public BasicDataSource(String driver, String connectURI, String uname, String passwd, int maxActive, int maxIdle, int idleTimeOut, int idleTestPeriod, boolean poolPreparedStatements) {
        ...
        DriverManagerConnectionFactory connectionFactory = new DriverManagerConnectionFactory(connectURI, connectionProperties);
        PoolableConnectionFactory poolableConnectionFactory = new PoolableConnectionFactory((ConnectionFactory)connectionFactory, (ObjectPool)connectionPool, (KeyedObjectPoolFactory)statementPoolFactory, "SELECT 1", false, true);
    ...
    public Connection getConnection(Connection con) throws SQLException {
        return con == null || con.isClosed() ? (con = this._source.getConnection()) : con;
    }
