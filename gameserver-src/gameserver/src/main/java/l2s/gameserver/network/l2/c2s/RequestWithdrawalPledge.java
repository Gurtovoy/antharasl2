...
extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (activeChar.isInCombat()) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_LEAVE_A_CLAN_WHILE_ENGAGED_IN_COMBAT);
            return;
        }
        ...
        clan.broadcastToOnlineMembers(new IBroadcastPacket[]{new SystemMessagePacket(SystemMsg.S1_HAS_WITHDRAWN_FROM_THE_CLAN).addString(activeChar.getName()), new PledgeShowMemberListDeletePacket(activeChar.getName())});
        if (subUnitType == -1) {
