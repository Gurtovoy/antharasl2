...
public class ArabicConv {
    ...
    private static String shapeArabic0(String src) {
        ...
        switch (src.length()) {
            ...
            case 1: {
                return new String(new char[]{ArabicConv.getReshapedGlphy(src.charAt(0), 0)});
            }
            case 2: {
                char lam = src.charAt(0);
                char alif = src.charAt(1);
                char lam_alif2 = ArabicConv.getLamAlef(alif, lam, true);
                if (lam_alif2 <= '\u0000') break;
        ...
        reshapedLetters[0] = ArabicConv.getReshapedGlphy(currLetter, 2);
        for (int i = 1; i < src.length() - 1; ++i) {
            lam_alif = ArabicConv.getLamAlef(src.charAt(i), currLetter, true);
            if (lam_alif > '\u0000') {
                if (i - 2 < 0 || i - 2 >= 0 && ArabicConv.getGlphyType(src.charAt(i - 2)) == '\u0002') {
                    reshapedLetters[i - 1] = '\u0000';
                    reshapedLetters[i] = lam_alif;
                } else {
                    reshapedLetters[i - 1] = '\u0000';
                    reshapedLetters[i] = ArabicConv.getLamAlef(src.charAt(i), currLetter, false);
                }
            } else {
                reshapedLetters[i] = ArabicConv.getGlphyType(src.charAt(i - 1)) == '\u0002' ? ArabicConv.getReshapedGlphy(src.charAt(i), 2) : ArabicConv.getReshapedGlphy(src.charAt(i), 3);
            }
            currLetter = src.charAt(i);
        }
        int len = src.length();
        lam_alif = ArabicConv.getLamAlef(src.charAt(len - 1), src.charAt(len - 2), true);
        if (lam_alif > '\u0000') {
            if (len > 3 && ArabicConv.getGlphyType(src.charAt(len - 3)) == '\u0002') {
                reshapedLetters[len - 2] = '\u0000';
                reshapedLetters[len - 1] = lam_alif;
            } else {
                reshapedLetters[len - 2] = '\u0000';
                reshapedLetters[len - 1] = ArabicConv.getLamAlef(src.charAt(len - 1), src.charAt(len - 2), false);
            }
        } else {
            reshapedLetters[len - 1] = ArabicConv.getGlphyType(src.charAt(len - 2)) == '\u0002' ? ArabicConv.getReshapedGlphy(src.charAt(len - 1), 1) : ArabicConv.getReshapedGlphy(src.charAt(len - 1), 4);
        }
    ...
    public static String shapeArabic(String src) {
        ...
        int len = src.length();
        for (int i = 0; i < len; ++i) {
            if (ArabicConv.isArChar(src.charAt(i))) {
                int arStart = i;
                while (i < len && ArabicConv.isArChar(src.charAt(i))) {
                    ++i;
                }
                sb.append(ArabicConv.shapeArabic0(src.substring(arStart, i)));
                if (i >= len) continue;
    ...
    public static final void main(String ... args) {
        System.out.println(ArabicConv.shapeArabic("adfa\u043e\u0440\u0444\u044b\u0432\u0644\u0627\u0634\u0633\u062a\u064a\u0627\u0644\u0627\u0634\u0633\u0631\u064a\u0644\u0634\u0631 \u0634\u0644\u0635\u0627\u064a\u0631 \u0644\u0631\u064a\u0635"));
        System.out.println(ArabicConv.shapeArabic("awdhgb \u0634\u0644\u0634\u0633 \u0644\u0633\u0634hgasv\u0440\u043f\u0444\u044b\u043c\u0432 \u0644\u0631\u0634\u06332323\u064a\u0644\u0633\u0634\u0627\u0631"));
        System.out.println(ArabicConv.shapeArabic("dashd\u0627\u062a\u0644\u0627\u064a \u062a\u0627\u0633\u064a \u062a\u0627\u064a \u0627\u062a\u0634\u0633\u0631 \u0635\u0639\u063a \u063a\u0644\u064a\t \u0636\u063a\u0639\u0644\u064a\u0635\u0636 dsaugd"));
    }

    static {
        HARAKATE = new char[]{'\u064b', '\u064c', '\u064d', '\u064e', '\u064f', '\u0650', '\u0651', '\u0652', '\u0653', '\u0654', '\u0655', '\u0656'};
        LAM_ALEF_GLPHIES = new char[][]{{'\u3ba6', '\ufef6', '\ufef5'}, {'\u3ba7', '\ufef8', '\ufef7'}, {'\u0625', '\ufefa', '\ufef9'}, {'\u0627', '\ufefc', '\ufefb'}};
        HashMap<Character, char[]> arabivGlphiesMap = new HashMap<Character, char[]>();
