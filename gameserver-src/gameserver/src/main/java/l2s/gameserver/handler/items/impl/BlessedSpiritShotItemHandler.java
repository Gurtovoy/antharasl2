...
public class BlessedSpiritShotItemHandler
extends DefaultItemHandler {
    ...
    public boolean useItem(Playable playable, ItemInstance item, boolean ctrl) {
        ...
        if (player.getActiveWeaponInstance() == null) {
            if (!isAutoSoulShot) {
                player.sendPacket((IBroadcastPacket)SystemMsg.YOU_MAY_NOT_USE_SPIRITSHOTS);
            }
            return false;
        }
        ...
        if (reducedSpiritshot[0] > 0 && Rnd.chance((int)reducedSpiritshot[0])) {
        ...
        if (grade != item.getGrade()) {
            ...
            player.sendPacket((IBroadcastPacket)SystemMsg.YOUR_SPIRITSHOT_DOES_NOT_MATCH_THE_WEAPONS_GRADE);
            return false;
        }
        if (!player.getInventory().destroyItem(item, bspsConsumption)) {
            ...
            player.sendPacket((IBroadcastPacket)SystemMsg.YOU_DO_NOT_HAVE_ENOUGH_SPIRITSHOT_FOR_THAT);
            return false;
        }
