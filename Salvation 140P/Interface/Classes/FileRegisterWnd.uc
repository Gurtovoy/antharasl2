...
function FileRegisterWndShowByTypeStr(string numStr) 
{
	...
	switch(numStr)
	{
		case "FH_PLEDGE_CREST_UPLOAD": ShowFileRegisterWnd(_FileHandler.FH_PLEDGE_CREST_UPLOAD); break;
		case "FH_PLEDGE_EMBLEM_UPLOAD"  : ShowFileRegisterWnd(_FileHandler.FH_PLEDGE_EMBLEM_UPLOAD); break;
		case "FH_ALLIANCE_CREST_UPLOAD" : ShowFileRegisterWnd(_FileHandler.FH_ALLIANCE_CREST_UPLOAD); break;
		case "FH_WEBBROWSER_FILE_UPLOAD": ShowFileRegisterWnd(_FileHandler.FH_WEBBROWSER_FILE_UPLOAD); break;

		default : Debug("Error: FileRegisterWndShowByNum 함수에 잘못된 번호를 넣음:" @ numStr);
	}
}
