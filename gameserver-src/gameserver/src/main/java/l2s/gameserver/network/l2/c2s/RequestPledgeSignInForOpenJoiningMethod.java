...
extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (activeChar.isInTrainingCamp()) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_JOIN_A_CLAN_WHILE_YOU_ARE_IN_THE_TRAINING_CAMP);
            return;
        }
        if (!activeChar.canJoinClan()) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.AFTER_LEAVING_OR_HAVING_BEEN_DISMISSED_FROM_A_CLAN_YOU_MUST_WAIT_AT_LEAST_A_DAY_BEFORE_JOINING_ANOTHER_CLAN);
            return;
        }
        ...
        if (!clan.checkJoinPledgeCondition(activeChar, pledgeType)) {
            if (pledgeType == -1) {
                if (activeChar.isAcademyGraduated()) {
                    activeChar.sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.S1_ALREADY_GRADUATED_FROM_A_CLAN_ACADEMY_THEREFORE_REJOINING_IS_NOT_ALLOWED).addName(activeChar));
                } else {
        ...
        activeChar.sendPacket((IBroadcastPacket)new ExPledgeRecruitBoardDetail(clanSearch));
    }
}
