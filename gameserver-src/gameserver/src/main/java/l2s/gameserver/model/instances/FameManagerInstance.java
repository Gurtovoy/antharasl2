...
public class FameManagerInstance
extends NpcInstance {
        ...
        if (actualCommand.equalsIgnoreCase("PK_Count")) {
            if (player.getFame() >= 5000) {
                if (player.getPkKills() > 0) {
                    ...
                    html.setFile("default/" + this.getNpcId() + "-okpk.htm");
                } else {
                    html.setFile("default/" + this.getNpcId() + "-nohavepk.htm");
                }
            } else {
                html.setFile("default/" + this.getNpcId() + "-nofame.htm");
            }
            ...
            if (player.getClan() == null || player.getClassLevel().ordinal() < ClassLevel.SECOND.ordinal() || player.getClan().getLevel() < 5) {
                html.setFile("default/" + this.getNpcId() + "-noclancrp.htm");
                ...
                player.sendPacket((IBroadcastPacket)SystemMsg.YOU_HAVE_ACQUIRED_50_CLAN_FAME_POINTS);
                html.setFile("default/" + this.getNpcId() + "-okclancrp.htm");
            }
            html.replace("%objectId%", String.valueOf(this.getObjectId()));
            player.sendPacket((IBroadcastPacket)html);
        } else {
