...
extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        if (activeChar.getParty().getCommandChannel().getChannelLeader() != activeChar) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.ONLY_THE_CREATOR_OF_A_COMMAND_CHANNEL_CAN_ISSUE_A_GLOBAL_COMMAND);
            return;
        }
        target.getParty().getCommandChannel().getChannelLeader().sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.C1S_PARTY_HAS_BEEN_DISMISSED_FROM_THE_COMMAND_CHANNEL).addName(target));
