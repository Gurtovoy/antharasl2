...
extends L2GameClientPacket {
    ...
    protected boolean readImpl() {
        ...
        this._type = (ChatType)((Object)ArrayUtils.valid((Object[])ChatType.VALUES, (int)this.readD()));
    ...
    protected void runImpl() {
        ...
        Say2C.writeToChat(activeChar, this._text, this._type, this._target);
    }
    ...
    public static void writeToChat(Player activeChar, String text, ChatType type, String target) {
        ...
        // Block fake players (autobots and fake players) from sending any chat messages
        if (activeChar.isFakePlayer()) {
            return;
        }
        ...
        if (Config.BAN_FOR_CFG_USAGE && (text.startsWith("//cfg") || text.startsWith("///cfg") || text.startsWith("////cfg"))) {
            activeChar.kick();
        }
        if (text.startsWith(".")) {
            ...
            if (!expression.isEmpty()) {
                try {
                    expr = ExpressionTree.parse((String)expression);
                }
                ...
                if (expr != null) {
                    try {
                        ...
                        vm.setValue("adena", (double)activeChar.getAdena());
        ...
        if (activeChar.isChatBlocked()) {
            activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_ARE_NOT_ALLOWED_TO_CHAT_WITH_A_CONTACT_WHILE_A_CHATTING_BLOCK_IS_IMPOSED);
            return;
        }
        boolean bl = globalchat = type != ChatType.ALLIANCE && type != ChatType.CLAN && type != ChatType.PARTY;
        if (globalchat || org.apache.commons.lang3.ArrayUtils.contains((int[])Config.BAN_CHANNEL_LIST, (int)type.ordinal())) {
            ...
            if (banInfo != null) {
                if (banInfo.getEndTime() == -1) {
                    ...
                    activeChar.sendMessage(new CustomMessage("common.ChatBanned").addString(TimeUtils.toSimpleFormat((long)banInfo.getEndTime() * 1000L)));
                }
                activeChar.sendActionFailed();
                return;
            }
        ...
        if ((translit = activeChar.getVar("translit")) != null) {
            ...
            while (m.find()) {
                ...
                sb.append(Strings.fromTranslit(text.substring(n, end), translit.equals("tl") ? 1 : 2));
                int n2 = end;
                end = m.end();
                sb.append(text.substring(n2, end));
            }
        ...
        switch (type) {
            case TELL: {
                ...
                if (receiver != null && receiver.isInOfflineMode()) {
                    activeChar.sendPacket(new IBroadcastPacket[]{new SystemMessagePacket(SystemMsg.S1_IS_NOT_CURRENTLY_LOGGED_IN).addString(target), ActionFailPacket.STATIC});
                    break;
                }
                    ...
                    if (!receiver.getMessageRefusal()) {
                        ...
                        if (activeChar.canTalkWith(receiver)) {
                            ...
                            if (receiver.isFakePlayer()) {
                                ...
                                receiver.sendPacket((IBroadcastPacket)cs);
                            }
            ...
            case SHOUT: {
                if (activeChar.isInObserverMode()) {
                    activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_CHAT_WHILE_IN_OBSERVATION_MODE);
                    return;
                }
            ...
            case ALL: {
                ...
                if (activeChar.isInOlympiadMode() && (game = activeChar.getOlympiadGame()) != null) {
            ...
            case PARTY_ROOM: {
                ...
                for (Player roomMember : room.getPlayers()) {
                    if (!activeChar.canTalkWith(roomMember)) continue;
                    roomMember.sendPacket((IBroadcastPacket)cs);
                }
                break;
            }
            case COMMANDCHANNEL_ALL: {
                if (!activeChar.isInParty() || !activeChar.getParty().isInCommandChannel()) {
                    activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_DO_NOT_HAVE_THE_AUTHORITY_TO_USE_THE_COMMAND_CHANNEL);
                    return;
                }
                ...
                activeChar.sendPacket((IBroadcastPacket)SystemMsg.ONLY_THE_COMMAND_CHANNEL_CREATOR_CAN_USE_THE_RAID_LEADER_TEXT);
                break;
            }
            case COMMANDCHANNEL_COMMANDER: {
                ...
                activeChar.sendPacket((IBroadcastPacket)SystemMsg.ONLY_A_PARTY_LEADER_CAN_ACCESS_THE_COMMAND_CHANNEL);
                break;
            }
            case HERO_VOICE: {
                ...
                if (!activeChar.getPlayerAccess().CanAnnounce && !activeChar.getAntiFlood().canHero(text)) {
                    return;
                }
                ChatUtils.announce(activeChar, cs);
                activeChar.sendPacket((IBroadcastPacket)cs);
            ...
            case PETITION_GM: {
                if (!PetitionManager.getInstance().isPlayerInConsultation(activeChar)) {
                    activeChar.sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.YOU_ARE_CURRENTLY_NOT_IN_A_PETITION_CHAT));
                    return;
                }
            ...
            case BATTLEFIELD: {
                ...
                for (Player player : GameObjectsStorage.getPlayers(false, false)) {
                    if (player.getBlockList().contains(activeChar) || player.isBlockAll() || !activeChar.canTalkWith(player) || player.getBattlefieldChatId() != activeChar.getBattlefieldChatId()) continue;
