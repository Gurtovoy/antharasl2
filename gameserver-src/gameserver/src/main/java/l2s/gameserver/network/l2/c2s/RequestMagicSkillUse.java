...
extends L2GameClientPacket {
    ...
    protected void runImpl() {
        ...
        SkillEntry skillEntry = SkillEntry.makeSkillEntry(SkillEntryType.NONE, this._magicId, activeChar.getSkillLevel(this._magicId));
        if (skillEntry != null) {
            ...
            if (skill.isToggle() && activeChar.getAbnormalList().contains(skill)) {
                if (!skill.isNecessaryToggle()) {
                    if (activeChar.isSitting()) {
                        activeChar.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_MOVE_WHILE_SITTING);
                        return;
                    }
            ...
            activeChar.getAI().Cast(skillEntry, target, this._ctrlPressed, this._shiftPressed);
        } else {
            activeChar.sendActionFailed();
        }
    }
