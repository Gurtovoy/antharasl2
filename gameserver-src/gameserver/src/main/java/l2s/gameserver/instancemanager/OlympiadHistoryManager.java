...
public class OlympiadHistoryManager {
    ...
    OlympiadHistoryManager() {
        ...
        for (Map.Entry<Boolean, List<OlympiadHistory>> entry : historyList.entrySet()) {
            for (OlympiadHistory history : entry.getValue()) {
                this.addHistory(entry.getKey(), history);
            }
        }
    }

    public void switchData() {
        this._historyOld.clear();
        this._historyOld.putAll(this._historyNew);
        this._historyNew.clear();
        OlympiadHistoryDAO.getInstance().switchData();
    }

    public void saveHistory(OlympiadHistory history) {
        this.addHistory(false, history);
        OlympiadHistoryDAO.getInstance().insert(history);
    }

    public void addHistory(boolean old, OlympiadHistory history) {
        IntObjectMap<List<OlympiadHistory>> map = old ? this._historyOld : this._historyNew;
        this.addHistory0(map, history.getObjectId1(), history);
        this.addHistory0(map, history.getObjectId2(), history);
    }
    ...
    private void addHistory0(IntObjectMap<List<OlympiadHistory>> map, int objectId, OlympiadHistory history) {
        ...
        if (historySet == null) {
            historySet = new CopyOnWriteArrayList<OlympiadHistory>();
    ...
    public void showHistory(Player player, int targetClassId, int page) {
        ...
        List<OlympiadHistory> historyList = (List<OlympiadHistory>)this._historyOld.get(entry.getKey());
        if (historyList == null) {
        ...
        for (OlympiadHistory h : historyList) {
            ...
            int n = team = entry.getKey() == h.getObjectId1() ? 1 : 2;
        ...
        for (int i = 0; i < historyList.size(); ++i) {
            ...
            if (history.getGameStatus() == 0) {
                ...
                int team;
                int n = team = entry.getKey() == history.getObjectId1() ? 1 : 2;
