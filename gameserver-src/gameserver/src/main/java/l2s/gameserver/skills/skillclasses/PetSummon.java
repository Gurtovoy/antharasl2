...
extends Skill {
    ...
    public boolean checkCondition(SkillEntry skillEntry, Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first, boolean sendMsg, boolean trigger) {
        ...
        if (player.isInCombat()) {
            player.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_SUMMON_DURING_COMBAT);
            return false;
        }
        ...
        if (player.isInBoat()) {
            player.sendPacket((IBroadcastPacket)SystemMsg.YOU_MAY_NOT_CALL_FORTH_A_PET_OR_SUMMONED_CREATURE_FROM_THIS_LOCATION);
            return false;
        }
        ...
        if (player.isInOlympiadMode()) {
            player.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_USE_THAT_ITEM_IN_A_GRAND_OLYMPIAD_MATCH);
            return false;
        }
        for (GameObject o : World.getAroundObjects(player, 120, 200)) {
            if (!o.isDoor()) continue;
            player.sendPacket((IBroadcastPacket)SystemMsg.YOU_MAY_NOT_SUMMON_FROM_YOUR_CURRENT_LOCATION);
            return false;
        }
        return true;
    }
