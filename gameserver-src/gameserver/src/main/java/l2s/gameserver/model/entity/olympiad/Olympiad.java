...
import l2s.gameserver.model.entity.olympiad.CompEndTask;
import l2s.gameserver.model.entity.olympiad.CompStartTask;
import l2s.gameserver.model.entity.olympiad.CompType;
import l2s.gameserver.model.entity.olympiad.OlympiadDatabase;
import l2s.gameserver.model.entity.olympiad.OlympiadEndTask;
...
import l2s.gameserver.model.entity.olympiad.Stadia;
import l2s.gameserver.model.entity.olympiad.ValidationTask;
import l2s.gameserver.model.entity.olympiad.WeeklyTask;
...
public class Olympiad {
    ...
    public static TIntList _nonClassBasedRegisters = new TSynchronizedIntList((TIntList)new TIntArrayList());
    ...
    public static final int[][] BUFFS_LIST = new int[][]{{4357, 1}, {4355, 1}, {4342, 1}, {4345, 1}, {4344, 1}, {4349, 1}, {4347, 1}, {4348, 1}, {4352, 1}};
    ...
    private static final Calendar _compStart;
    ...
    private static List<NpcInstance> _npcs;
    ...
    public static void load() {
        ...
        switch (_period) {
            ...
            case 1: {
                _isOlympiadEnd = true;
                _scheduledValdationTask = ThreadPoolManager.getInstance().schedule(new ValidationTask(), Olympiad.getMillisToValidationEnd());
                break;
            }
            default: {
        ...
        long milliToEnd = _period == 0 ? Olympiad.getMillisToOlympiadEnd() : Olympiad.getMillisToValidationEnd();
        double numSecs = milliToEnd / 1000L % 60L;
        double countDown = ((double)(milliToEnd / 1000L) - numSecs) / 60.0;
        ...
        if (_period == 0) {
            ...
            milliToEnd = Olympiad.getMillisToWeekChange();
            double numSecs2 = milliToEnd / 1000L % 60L;
            double countDown2 = ((double)(milliToEnd / 1000L) - numSecs2) / 60.0;
            int numMins2 = (int)Math.floor(countDown2 % 60.0);
            countDown2 = (countDown2 - (double)numMins2) / 60.0;
            int numHours2 = (int)Math.floor(countDown2 % 24.0);
            int numDays2 = (int)Math.floor((countDown2 - (double)numHours2) / 24.0);
    ...
    private static void initStadiums() {
        for (int i = 0; i < STADIUMS.length; ++i) {
            if (STADIUMS[i] != null) continue;
            Olympiad.STADIUMS[i] = new Stadia();
        }
    }
    ...
    public static void init() {
        ...
        Olympiad.startOlympiadEndTask(Olympiad.getMillisToOlympiadEnd());
        Olympiad.updateCompStatus();
        if (_scheduledWeeklyTask != null) {
            _scheduledWeeklyTask.cancel(false);
        }
        _scheduledWeeklyTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new WeeklyTask(), Olympiad.getMillisToWeekChange(), Config.ALT_OLY_WPERIOD);
    }
    ...
    public static boolean isClassedBattlesAllowed() {
        return Config.CLASSED_GAMES_ENABLED && Olympiad.getCompWeek() == 4;
    }
    ...
    public static boolean isRegistrationActive() {
        ...
        if (Olympiad.getMillisToOlympiadEnd() <= 600000L) {
            return false;
        }
        return Olympiad.getMillisToCompEnd() > Config.OLYMPIAD_REGISTRATION_DELAY;
    }
    ...
    public static synchronized boolean registerParticipant(Player player, CompType type) {
        ...
        if (!Olympiad.validPlayer(player, player, type, false)) {
            return false;
        }
        ...
        switch (type) {
            case CLASSED: {
                if (!Olympiad.isClassedBattlesAllowed()) break;
                _classBasedRegisters.put(ClassId.VALUES[Olympiad.getParticipantClass(player.getObjectId())].getType2().ordinal(), player.getObjectId());
                if (Config.ALT_OLY_BY_SAME_BOX_NUMBER > 0) {
                    _playersHWID.put(player.getObjectId(), player.getNetConnection().getHWID());
                }
                player.sendPacket((IBroadcastPacket)SystemMsg.YOU_HAVE_BEEN_REGISTERED_FOR_THE_GRAND_OLYMPIAD_WAITING_LIST_FOR_A_CLASS_SPECIFIC_MATCH);
                break;
            }
            case NON_CLASSED: {
                ...
                player.sendPacket((IBroadcastPacket)SystemMsg.YOU_ARE_CURRENTLY_REGISTERED_FOR_A_1V1_CLASS_IRRELEVANT_MATCH);
            }
        }
        return true;
    }
    ...
    public static boolean validPlayer(Player sendPlayer, Player validPlayer, CompType type, boolean gameValidation) {
        ...
        if (!validPlayer.isBaseClassActive()) {
            sendPlayer.sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.C1_DOES_NOT_MEET_THE_PARTICIPATION_REQUIREMENTS_SUBCLASS_CHARACTER_CANNOT_PARTICIPATE_IN_THE_OLYMPIAD).addName(validPlayer));
            return false;
        }
        if (validPlayer.isFishing()) {
            if (validPlayer == sendPlayer) {
                sendPlayer.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_PARTICIPATE_IN_THE_OLYMPIAD_WHILE_FISHING);
            }
            return false;
        }
        if (validPlayer.isInTrainingCamp()) {
            sendPlayer.sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.C1_IS_CURRENTLY_IN_THE_ROYAL_TRAINING_CAMP_AND_CANNOT_PARTICIPATE_IN_THE_OLYMPIAD).addName(validPlayer));
            return false;
        }
        if (validPlayer.containsEvent(SingleMatchEvent.class)) {
            if (validPlayer == sendPlayer) {
                sendPlayer.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_BE_SIMULTANEOUSLY_REGISTERED_FOR_PVP_MATCHES_SUCH_AS_THE_OLYMPIAD_UNDERGROUND_COLISEUM_AERIAL_CLEFT_KRATEIS_CUBE_AND_HANDYS_BLOCK_CHECKERS);
            }
            return false;
        }
        ...
        Olympiad.addParticipant(validPlayer);
        if (!gameValidation) {
            int[] ar = Olympiad.getWeekGameCounts(validPlayer.getObjectId());
            switch (type) {
                case CLASSED: {
                    if (_classBasedRegisters.containsValue(validPlayer.getObjectId())) {
                        sendPlayer.sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.C1_IS_ALREADY_REGISTERED_ON_THE_CLASS_MATCH_WAITING_LIST).addName(validPlayer));
                        return false;
                    }
                    if (ar[1] != 0) break;
                    validPlayer.sendPacket((IBroadcastPacket)SystemMsg.THE_TOTAL_NUMBER_OF_MATCHES_THAT_CAN_BE_ENTERED_IN_1_WEEK_IS_60_CLASS_IRRELEVANT_INDIVIDUAL_MATCHES_30_SPECIFIC_MATCHES_AND_10_TEAM_MATCHES);
                    return false;
                }
                case NON_CLASSED: {
                    if (_nonClassBasedRegisters.contains(validPlayer.getObjectId())) {
                        sendPlayer.sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.C1_IS_ALREADY_REGISTERED_ON_THE_WAITING_LIST_FOR_THE_CLASS_IRRELEVANT_INDIVIDUAL_MATCH).addName(validPlayer));
                        return false;
                    }
                    if (ar[2] != 0) break;
                    validPlayer.sendPacket((IBroadcastPacket)SystemMsg.THE_TOTAL_NUMBER_OF_MATCHES_THAT_CAN_BE_ENTERED_IN_1_WEEK_IS_60_CLASS_IRRELEVANT_INDIVIDUAL_MATCHES_30_SPECIFIC_MATCHES_AND_10_TEAM_MATCHES);
            ...
            if (ar[0] == 0) {
                validPlayer.sendPacket((IBroadcastPacket)SystemMsg.THE_MAXIMUM_MATCHES_YOU_CAN_PARTICIPATE_IN_1_WEEK_IS_70);
                return false;
            }
            if (Olympiad.isRegisteredInComp(validPlayer)) {
                sendPlayer.sendPacket((IBroadcastPacket)new SystemMessagePacket(SystemMsg.C1_IS_ALREADY_REGISTERED_ON_THE_MATCH_WAITING_LIST).addName(validPlayer));
                return false;
            }
        }
        return true;
    ...
    public static synchronized void logoutPlayer(Player player) {
        ...
        _nonClassBasedRegisters.remove(new Integer(player.getObjectId()).intValue());
    ...
    public static synchronized boolean unregisterParticipant(Player player) {
        ...
        if (!Olympiad.isRegistered(player, true)) {
            player.sendPacket((IBroadcastPacket)SystemMsg.YOU_ARE_NOT_CURRENTLY_REGISTERED_FOR_THE_GRAND_OLYMPIAD);
            return false;
        }
        OlympiadGame game = player.getOlympiadGame();
        if (game != null) {
        ...
        _nonClassBasedRegisters.remove(new Integer(player.getObjectId()).intValue());
        _playersHWID.remove(player.getObjectId());
        player.sendPacket((IBroadcastPacket)SystemMsg.YOU_HAVE_BEEN_REMOVED_FROM_THE_GRAND_OLYMPIAD_WAITING_LIST);
        return true;
    }

    private static synchronized void updateCompStatus() {
        long milliToStart = Olympiad.getMillisToCompBegin();
        ...
        Olympiad.startCompStartTask(Olympiad.getMillisToCompBegin());
    }
    ...
    private static long getMillisToOlympiadEnd() {
        return Math.max(10L, Olympiad.getOlympiadPeriodEndTime() - System.currentTimeMillis());
    }
    ...
    static long getMillisToValidationEnd() {
        return Math.max(10L, Olympiad.getValidationEndTime() - System.currentTimeMillis());
    }
    ...
    private static long getMillisToCompBegin() {
        if (_compStart.getTimeInMillis() < System.currentTimeMillis()) {
            ...
            return Olympiad.setNewCompBegin();
        }
    ...
    private static long getMillisToWeekChange() {
        return Math.max(10L, Olympiad.getWeekEndTime() - System.currentTimeMillis());
    }
    ...
    public static synchronized void doWeekTasks() {
        ...
        for (IntObjectPair entry : _participants.entrySet()) {
            OlympiadParticipiantData data = (OlympiadParticipiantData)entry.getValue();
            ...
            player.sendPacket((IBroadcastPacket)((SystemMessagePacket)new SystemMessagePacket(SystemMsg.C1_HAS_EARNED_S2_POINTS_IN_THE_GRAND_OLYMPIAD_GAMES).addName(player)).addInteger(Config.OLYMPIAD_POINTS_WEEKLY));
        }
    }
    ...
    public static synchronized void addObserver(int id, Player observer) {
        if (observer.getOlympiadGame() != null || Olympiad.isRegistered(observer, false) || Olympiad.isRegisteredInComp(observer)) {
            observer.sendPacket((IBroadcastPacket)SystemMsg.YOU_MAY_NOT_OBSERVE_A_GRAND_OLYMPIAD_GAMES_MATCH_WHILE_YOU_ARE_ON_THE_WAITING_LIST);
            return;
        }
        if (_manager == null || _manager.getOlympiadInstance(id) == null || _manager.getOlympiadInstance(id).getStatus() == BattleStatus.Begining || _manager.getOlympiadInstance(id).getStatus() == BattleStatus.Begin_Countdown) {
            observer.sendPacket((IBroadcastPacket)SystemMsg.THE_GRAND_OLYMPIAD_GAMES_ARE_NOT_CURRENTLY_IN_PROGRESS);
            return;
        }
        if (observer.isInCombat() || observer.getPvpFlag() > 0 || observer.containsEvent(SingleMatchEvent.class)) {
            observer.sendPacket((IBroadcastPacket)SystemMsg.YOU_CANNOT_OBSERVE_WHILE_YOU_ARE_IN_COMBAT);
            return;
        }
        for (Servitor servitor : observer.getServitors()) {
            servitor.unSummon(false);
        }
        observer.enterArenaObserverMode(Olympiad.getOlympiadGame(id));
    }
    ...
    public static synchronized int getParticipantRewardCount(Player player, boolean remove) {
        ...
        OlympiadParticipiantData participant = Olympiad.getParticipantInfo(objId);
        if (participant == null) {
            return 0;
        }
    ...
    public static synchronized boolean isRegistered(Player player) {
        return Olympiad.isRegistered(player, false);
    }
    ...
    public static synchronized boolean isRegistered(Player player, boolean unregister) {
        ...
        if (Config.ALT_OLY_BY_SAME_BOX_NUMBER > 0 && !unregister) {
            GameClient client = player.getNetConnection();
            if (client == null) {
                return true;
            }
            String playerHWID = client.getHWID();
            if (StringUtils.isEmpty((CharSequence)playerHWID) || !_playersHWID.containsValue(playerHWID)) {
                return false;
            }
            int boxesCount = 0;
            for (String hwid : _playersHWID.values()) {
                if (hwid == null || !hwid.equals(playerHWID)) continue;
                ++boxesCount;
            }
    ...
    public static synchronized boolean isRegisteredInComp(Player player) {
        ...
        for (OlympiadGame g : _manager.getOlympiadGames().values()) {
            if (g == null || !g.isRegistered(player.getObjectId())) continue;
            return true;
        }
        return false;
    }
    ...
    public static OlympiadParticipiantData getParticipantInfo(int objectId) {
        return (OlympiadParticipiantData)_participants.get(objectId);
    }
    ...
    public static int convertParticipantClassId(int baseClassId) {
        ...
        if (classId.getClassLevel().ordinal() < ClassLevel.SECOND.ordinal()) {
            for (ClassId id : ClassId.VALUES) {
                if (!id.isOfLevel(ClassLevel.SECOND) || !id.childOf(classId)) continue;
                return id.getId();
            }
        }
    ...
    public static synchronized void addParticipant(Player participant) {
        OlympiadParticipiantData participantData = (OlympiadParticipiantData)_participants.get(participant.getObjectId());
    ...
    static {
        ...
        STADIUMS = new Stadia[Config.OLYMPIAD_STADIAS_COUNT];
        _npcs = new ArrayList<NpcInstance>();
    }
}
