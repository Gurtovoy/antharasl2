class InviteClanPopWndClassic extends UICommonAPI;
...
function OnLoad()
{
	...
	Me = GetWindowHandle( "InviteClanPopWndClassic" );
...
function AskJoin()
{
	...
	if( GetTargetInfo( user ) )
	{
		if( user.nID > 0 )
		{
			index = class'UIAPI_COMBOBOX'.static.GetSelectedNum("InviteClanPopWndClassic.ComboboxInviteClandPopWnd");
			if( index >= 0 )
			{
				knighthoodID = class'UIAPI_COMBOBOX'.static.GetReserved("InviteClanPopWndClassic.ComboboxInviteClandPopWnd", index);
...
{
	local int i;	
	local ClanWndClassic scriptClassic;
	...
	class'UIAPI_COMBOBOX'.static.Clear("InviteClanPopWndClassic.ComboboxInviteClandPopWnd");
	
	scriptClassic = ClanWndClassic( GetScript("ClanWndClassic") );
	countnum2 = "" $ scriptClassic.m_myClanType;		
	...
	for( i=0 ; i < CLAN_KNIGHTHOOD_COUNT ; ++i )
	{
		...
		m_sName = scriptClassic.m_memberList[i].m_sName;
		cnt2 = len(countnum);
		...
		if( m_sName != "" )
		{
			...
			{
				class'UIAPI_COMBOBOX'.static.AddStringWithReserved("InviteClanPopWndClassic.ComboboxInviteClandPopWnd", m_sName, m_knighthoodIndex[i] );
				++addedCount;
			}
			else if (cnt1 <= cnt2)
			{
	...
	if( addedCount > 0 )
		class'UIAPI_COMBOBOX'.static.SetSelectedNum("InviteClanPopWndClassic.ComboboxInviteClandPopWnd", 0);		// 제일 처음 아이템이 보이도록
}
defaultproperties
{
}
