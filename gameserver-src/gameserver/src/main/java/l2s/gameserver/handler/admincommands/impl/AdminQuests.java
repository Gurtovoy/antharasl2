...
implements IAdminCommandHandler {
    ...
    public boolean useAdminCommand(Enum<?> comm, String[] wordList, String fullString, Player activeChar) {
        ...
        switch (command) {
            case admin_quests: {
                return AdminQuests.ShowQuestList(this.getTargetChar(wordList, 1, activeChar), activeChar);
            }
            case admin_quest: {
    ...
    private boolean cmd_Clear(Quest _quest, String[] wordList, Player activeChar) {
        ...
        qs.abortQuest();
        return AdminQuests.ShowQuestList(targetChar, activeChar);
    }
    ...
    private boolean cmd_Show(Quest _quest, String[] wordList, Player activeChar) {
        ...
        return AdminQuests.ShowQuestState(qs, activeChar);
    }
    ...
    private static boolean ShowQuestState(QuestState qs, Player activeChar) {
        ...
        replyMSG.append(fmtRow.sprintf(new Object[]{"STATE: ", "1", fmtSetButton.sprintf(new Object[]{id, "STATE", "$new_val", char_name, ""})}));
    ...
    private static boolean ShowQuestList(Player targetChar, Player activeChar) {
        ...
        for (QuestState qs : targetChar.getAllQuestsStates()) {
            if (qs == null || !qs.getQuest().isVisible(activeChar)) continue;
            replyMSG.append(fmtListRow.sprintf(new Object[]{qs.getQuest().getId(), targetChar.getName(), qs.getQuest().getName(), "1"}));
        }
